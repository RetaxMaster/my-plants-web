...
- **And, above those, one line breaking the whole proposal down at a glance.** *"⚠ 2 aplicados · 1 parcial · 1 ya registrado"* / *"⚠ 2 applied · 1 partial · 1 already recorded"*, so you can see the whole result before reading the reasons underneath it. It appears **only when something did not apply** — when everything landed, the individual confirmations already say so and a line repeating it would be noise on by far the most common outcome. The middle count matters more than it looks: a repotting the plant already had recorded for that day still updates the pot size and the soil, so it is genuinely *partial* — neither applied nor skipped — and counting it as one or the other would put the summary at odds with the very sentence sitting beneath it. Counts of zero are simply left out, so a proposal with nothing partial reads *"2 aplicados · 2 ya registrados"*.
...
- **A plant's page now says why the watering check disappeared — once, in one place, with one way back.** The Water task can offer to check the pot for you — *"¿Necesitas regar?"* — and that offer depends on the app loading your measuring instruments for that plant. When that load fails the button is correctly withdrawn (there is nothing honest the app could check with), but it used to be withdrawn *silently*: the only message on the page was about the reading history, ending with *"your readings are still saved"* — reassurance that reads as "nothing else is wrong", while the button you came for had gone with nothing accounting for it. The page now states the fact above the care tasks, with a **Reintentar** beside it that brings the check straight back when the connection recovers, and says out loud that you can still mark the watering done or postpone it, because you still can. **And it says it exactly once.** For a while the same failure produced two red notices about the same request, each with its own **Reintentar** — either one fixing both — and a screen reader was interrupted twice for one event. There is now a single notice: it names the instruments and the check when that is what you lost, and falls back to *"No pudimos cargar el historial de lecturas. Tus lecturas siguen guardadas."* when the check was never on offer for you anyway — because you have not told the app what you measure with, or because you already watered this plant today.
- **…and losing your readings no longer takes away your ability to add one.** The same failure used to remove **Agregar lectura** along with the list, which told you your readings were safe and, in the same breath, that you could no longer add to them — over a failure to *read*. The button stays. Pressing it retries the load first, so when the connection has recovered it simply opens the measuring dialog as usual and clears the notice on the way; if the load is still failing it opens nothing rather than showing you an empty instrument picker you could not save from.
- **Buttons inside a notice no longer stretch across the whole banner.** Every **Reintentar** and every link the app puts inside a coloured notice used to span the notice's full width — measured at 650 px against about 84 px of label — which reads as a greyed-out input rather than as something you can press. They are now the width of their own label, everywhere in the app at once: the plant page, the Today screen, Settings and the care findings.
- **When a repot you record does not move the soil clock, the app says so.** The date the app tracks as "when this pot was last filled" only ever moves forward now. If you log a repot you had forgotten — dated before one already recorded — the event is still saved to the plant's history, and the notice under the task tells you the soil clock stays where the more recent record put it, naming that date. Previously this happened silently: the clock jumped backwards, and every soil reading taken from the older date onwards was quietly withdrawn along with it.
- **…and it now also tells you when the pot size and soil you typed could not be applied.** Logging a repot dated before one the app already has keeps the event in your history but does not change the plant's current pot description — that older repot is not the pot it lives in today. The values you typed on the form were simply dropped, with nothing said. Now, when that record is also a second one for a day that already had it — the case where the app writes no new entry at all, so there is nowhere left to keep them — the notice says so in its own complete sentence, beside the two that were already there: *"The pot details were not applied: they describe an earlier repot, and your plant already has a more recent one."* It reads the same whether you recorded the repot yourself, from the plant's page or from Today, or approved one the Plant Doctor or the Gardener proposed. If you answered *"I don't know"* to both the diameter and the mix, nothing was lost and nothing is said.
- **…and it says so when the repot came from the Plant Doctor or the Gardener, too.** The notice above only ever appeared when *you* recorded the repot yourself. If an agent proposed one and you pressed **Aprobar**, the soil clock refused to move exactly the same way and you were told nothing at all — on the one path where you had the least visibility into what was about to happen. Approving a proposal now carries the same sentence, in the same place the rest of the approval result already appears, naming the same surviving date. It reads alongside the "already recorded" note rather than replacing it, since a repeat repot dated earlier than one you already had produces both facts at once.
