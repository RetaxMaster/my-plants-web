...
- **A plant's page now says why the watering check disappeared, instead of just dropping it.** The Water task can offer to check the pot for you — *"¿Necesitas regar?"* — and that offer depends on the app loading your measuring instruments for that plant. When that load fails, the button is correctly withdrawn: there is nothing honest the app could check with. But it was withdrawn *silently*. The only message anywhere on the page was about the reading history, and it ended with *"your readings are still saved"* — reassurance that reads as "nothing else is wrong", while the button you came for had gone with nothing accounting for it. The page now states the fact, above the care tasks, in the same words the Today screen already uses for this same failure, with a **Reintentar** beside it that brings the check straight back when the connection recovers — and it says out loud that you can still mark the watering done or postpone it, because you still can. It deliberately stays quiet in the two cases where nothing was taken away from you: if you have not told the app what you measure with (the offer was never there), and if the plant was already watered today (the question had already been answered).
