...
- **A dialog opened from another dialog now takes the keyboard with it.** Following "calíbrala" from the measuring dialog opened the calibration screen but left the keyboard behind it: Escape did nothing, and pressing Enter re-opened the measuring dialog *underneath* the one you were looking at, leaving two stacked panels. Focus now stays where the visible dialog is. Same fix covers a dialog opened directly by a link, which previously appeared on screen while the keyboard was still on the page behind it.
