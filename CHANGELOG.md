...
- **Instrument settings: picker and comparison table.** A new Settings page lets you see and adjust which instruments you own and measure with. A side-by-side comparison table shows each instrument's unit, whether its readings compare between pots, and whether it needs a per-pot calibration setup — so you can decide whether a reading is trustworthy for this particular plant.
